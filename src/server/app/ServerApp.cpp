...
void ServerApp::onUpdate(
    const std::shared_ptr<const ServerConfig>
        &config) { // It doesn't set any mutex yet but it will be once the app
...
float ServerApp::advanceClock(Clock::time_point &last,
...
void ServerApp::sleepRemainder(Clock::time_point frameStart) const {
  ...
  if (elapsed < m_loopSettings.tickDuration)
    std::this_thread::sleep_for(m_loopSettings.tickDuration - elapsed);
}
