...
struct GhostBehavior {
  ...
  [[nodiscard]] static core::maps::Tile
  blinkyTarget(const entt::registry &registry, const core::maps::Map &map);

  [[nodiscard]] static core::maps::Tile
  pinkyTarget(const entt::registry &registry, const core::maps::Map &map);

  // blinkyX/blinkyY — current pixel position of Blinky for the vector calculation.
  [[nodiscard]] static core::maps::Tile
  inkyTarget(const entt::registry &registry, const core::maps::Map &map,
             float blinkyX, float blinkyY);

  // clydeX/clydeY — current pixel position of Clyde for the distance check.
  [[nodiscard]] static core::maps::Tile
  clydeTarget(const entt::registry &registry, const core::maps::Map &map,
