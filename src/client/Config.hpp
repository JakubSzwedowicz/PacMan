...
struct ClientConfig {
    ...
    ClientConfig()
        : loggerConfig(m_container.buildConfigParam<Utils::Logging::LoggerConfig>(
          ...
          configPath(m_container.buildConfigParam<std::string>("config", "Path to client JSON config file",
                                                               "config/client.json")),
          windowWidth(m_container.buildConfigParam<int>("window_width", "Window width in pixels", 800)),
          windowHeight(m_container.buildConfigParam<int>("window_height", "Window height in pixels", 600)),
          playerName(m_container.buildConfigParam<std::string>("player_name", "Player display name", "Player")),
          serverAddress(m_container.buildConfigParam<std::string>("server_address", "Server IP address", "127.0.0.1")),
          serverPort(m_container.buildConfigParam<int>("server_port", "Server port number", 7777)),
          mapPath(m_container.buildConfigParam<std::string>("map_path", "Path to map JSON file",
