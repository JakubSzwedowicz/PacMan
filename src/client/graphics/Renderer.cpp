...
void Renderer::render(sf::RenderWindow &window, const entt::registry &registry,
  ...
  for (int row = 0; row < static_cast<int>(map.height); ++row) {
    for (int col = 0; col < static_cast<int>(map.width); ++col) {
      ...
      if (tile == '#') {
        ...
        rect.setPosition(
            {static_cast<float>(col) * ts, static_cast<float>(row) * ts});
  ...
  auto pacmanView =
      registry.view<const core::ecs::Position, const core::ecs::Collider,
