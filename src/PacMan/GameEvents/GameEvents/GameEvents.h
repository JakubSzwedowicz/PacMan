...
struct GameEvent : public BaseEvent {
  ...
  GameEvent(int gameRunnerId,
  GameEventType gameEventType) : gameRunnerId(gameRunnerId), gameEventType(gameEventType) {}
};
...
struct GameStatusChanged final : public GameEvent {
  ...
  explicit GameStatusChanged(int gameRunnerId, GameLogic::GameStatus gameStatus)
      : GameEvent(gameRunnerId, GameEventType::GAME_STATUS_CHANGED), gameStatus(gameStatus) {}
};
