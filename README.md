...
## Running
To run the game bothh the Server and a Client must be running. Client is just a relativly thin wrapper on the data server broadcasts with some entities movement prediction. Client spawns a Server automatically whenn hosting a game but a server can be also started manually with optional ascii rendering. So there are two options to start a game:
...
* Start the client with `./scripts/run_client.sh`, host thhe game, find the log file *clientLog.txt*, get the port number, start another client, insert the port number and join the game.
